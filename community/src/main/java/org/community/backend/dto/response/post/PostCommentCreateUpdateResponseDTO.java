...
public class PostCommentCreateUpdateResponseDTO extends ApiResponse {
    ...
    public static ResponseEntity<ApiResponse> commentNotFound() {
        ApiResponse response = new ApiResponse(ResponseCode.NOT_EXISTED_COMMENT, ResponseMessage.NOT_EXISTED_COMMENT);
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
    }
